...
run_script
if [[ "$OUTPUT" == "cached_token_abc" && "$EXIT_CODE" -eq 0 ]]; then
  pass "直接返回缓存 token，未调用登录 API"
else
  fail "应返回 cached_token_abc（exit=0），实际：$OUTPUT（exit=$EXIT_CODE）"
fi
...
mock_curl '{"unexpected": "curl_was_called"}'
run_script
if [[ "$OUTPUT" == "no_password_token" && "$EXIT_CODE" -eq 0 ]]; then
...
mock_curl '{"unexpected": "curl_was_called"}'
run_script
if [[ "$OUTPUT" == "auto_filled_token" && "$EXIT_CODE" -eq 0 ]]; then
  pass "URL/account 从缓存补全，直接返回缓存 token，无需密码"
else
  fail "应返回 auto_filled_token（exit=0），实际：$OUTPUT（exit=$EXIT_CODE）"
fi
...
run_script
if [[ "$OUTPUT" == "new_server_token" && "$EXIT_CODE" -eq 0 ]]; then
...
run_script
if [[ "$OUTPUT" == "new_account_token" && "$EXIT_CODE" -eq 0 ]]; then
...
run_script
if [[ "$OUTPUT" == "recovered_token" && "$EXIT_CODE" -eq 0 ]]; then
  pass "缓存损坏时自动降级重新登录"
else
...
mock_curl '{"unexpected": "curl_was_called"}'
EXIT_CODE=0
OUTPUT=$(ZENTAO_TOKEN="env_token_value" HOME="$FAKE_HOME" PATH="$MOCK_BIN:$PATH" bash "$SCRIPT" 2>&1) || EXIT_CODE=$?
if [[ "$OUTPUT" == "env_token_value" && "$EXIT_CODE" -eq 0 ]]; then
  pass "ZENTAO_TOKEN 直接返回，不调用登录 API"
else
  fail "应返回 env_token_value（exit=0），实际：$OUTPUT（exit=$EXIT_CODE）"
fi
...
if [[ "$OUTPUT" == "cache_wins_token" && "$EXIT_CODE" -eq 0 ]]; then
...
run_script
if [[ "$OUTPUT" == "nested_token" && "$EXIT_CODE" -eq 0 ]]; then
...
run_script
if [[ "$OUTPUT" == "toplevel_token" && "$EXIT_CODE" -eq 0 ]]; then
  pass "识别顶层 token 字段"
else
...
if [[ -f "$FAKE_HOME/.zentao-token.json" ]]; then
  ...
  if [[ "$_ct" == "written_token" && "$_cu" == "http://zentao.test" && "$_ca" == "admin" ]]; then
...
if [[ "$FIRST" == "first_login_token" && "$SECOND" == "first_login_token" && "$EXIT_CODE" -eq 0 ]]; then
  pass "第二次调用命中缓存，token 一致"
else
  fail "first=$FIRST second=$SECOND exit=$EXIT_CODE"
fi
